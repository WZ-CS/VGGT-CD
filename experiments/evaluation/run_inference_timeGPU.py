...
输出（每个场景）：
    {output_root}/{scene}/
        ...
        timing.json         # 包含不含网络推理的纯配准耗时 (paper_eval_time) 及峰值显存占用
...
def one_shot_svd_fine_alignment(points1, conf1, points2, conf2,
    ...
    print(f"  [Fine/SVD] s={fine_s:.4f}  corr={len(idx1):,}  t={elapsed:.4f}s")
...
def run_scene(scene_dir: str, seq_t1: str, seq_t2: str,
    ...
    print(f"  Coarse scale={coarse_s:.4f}  t={timing['coarse']:.4f}s")
    ...
    print(f"  Fine scale={fine_s:.4f}  t={timing['fine']:.4f}s")
    ...
    # 论文记录时间 = 仅包含粗配准和精配准的时间
    timing["paper_eval_time"] = timing["coarse"] + timing["fine"]
    # 完整流水线耗时 (含网络推理)
    timing["total_pipeline"] = timing.get("dense_inference", 0) + timing["paper_eval_time"]
    ...
    print(f"  >> Paper Eval Time (Coarse + Fine, Excl. Dense Infer): {timing['paper_eval_time']:.4f}s")
